...
export function LikeButton(props){
  ...
  async function handleLike(){
    if(!props.liked){
      try{
        let res = await PostAPI(`http://localhost:8000/posts/like/${props.id}`,{username:username});
        props.setLiked(true);
      ...
      try{
        let res = await PostAPI(`http://localhost:8000/posts/unlike/${props.id}`,{username:username});
  ...
  return(
    <div onClick={handleLike} className={classes.root}>
      <img src={src} />
      <p>{likeCount}</p>
    </div>
  )
