...
export function Comment(props){
  ...
  async function handleCommentDelete(){
    let found = false;
    try{
      let res = await PostAPI(`http://localhost:8000/posts/uncomment/${props.id}`,{username:UserName,commentId:props.comment.commentId});
      ...
      if(!found){
        props.setUserComments(props.userComments.filter((comment)=>{
          if(comment.commentId == props.comment.commentId){
            found = true
            return false;
